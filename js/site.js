...
(function () {
  ...
  /* ---------- Rangées magazine (accueil, prestations) ----------
     Entrée en fondu/glissé alternée au premier passage à l'écran. */
  var magRows = document.querySelectorAll('.mag-row');
  if (magRows.length) {
    var magObserver = new IntersectionObserver(function (entries) {
      entries.forEach(function (entry) {
        if (entry.isIntersecting) {
          entry.target.classList.add('in');
          magObserver.unobserve(entry.target);
        }
      });
    }, { threshold: 0.2 });
    magRows.forEach(function (row, i) {
      row.classList.add(i % 2 === 0 ? 'from-left' : 'from-right');
